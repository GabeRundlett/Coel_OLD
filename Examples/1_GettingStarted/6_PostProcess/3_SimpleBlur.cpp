...
int main() {
    ...
    Coel::Window window(800, 600, "Blurring Filter Example");
    ...
    void main() {
        ...
        color /= 4 * u_kernelSize * u_kernelSize;
        color.w = 1;
    }
    )";
    ...
    while (window.isOpen()) {
        ...
        quadShader.send(u_kernelSize, int(window.mouse.pos.x * 10 / window.size.x) + 1);
