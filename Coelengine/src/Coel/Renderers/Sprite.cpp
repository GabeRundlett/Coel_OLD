...
namespace Coel {
    ...
    void init(SpriteRenderer &s) {
        ...
        for (int32_t i = 0; i < 32; ++i)
            tids.at(static_cast<size_t>(i)) = i;
    ...
    void SpriteRenderer::submit(const std::array<Vertex, 4> &vertices) {
        ...
        *reinterpret_cast<std::array<Vertex, 4> *>(vertexHandle) = vertices;
    ...
    void SpriteRenderer::submit(const glm::vec4 &pos_size, const glm::vec4 &tex_pos_size, const Texture &texture,
                                const glm::vec4 &tint) {
        float tid = static_cast<float>(getIndexOf(texture));
    ...
    int32_t SpriteRenderer::getIndexOf(const Texture &texture) {
        for (int32_t i = 1; i < textureCount; ++i)
            if (textures[static_cast<size_t>(i)] && textures[static_cast<size_t>(i)]->id == texture.id)
                return i;

        textures[static_cast<size_t>(textureCount)] = &texture;
        ++textureCount;
    ...
    void SpriteRenderer::flush() {
        ...
        bind(shader);
        send(u_view, &viewMat);
        for (int32_t slot = 0; slot < textureCount; ++slot)
            bind(*textures[static_cast<size_t>(slot)], slot);
